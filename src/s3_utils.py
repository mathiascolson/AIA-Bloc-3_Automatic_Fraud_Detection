...
def get_s3_client():
    ...
    return boto3.client(
        ...
        region_name=get_settings.aws_default_region,
    )
...
def upload_file_to_s3(local_path: str | Path, s3_key: str) -> None:
    ...
    print(f"Uploaded to s3://{get_settings.s3_bucket_name}/{s3_key}")
...
def write_dataframe_to_s3_parquet(df: pd.DataFrame, s3_key: str) -> None:
    ...
    print(f"DataFrame written to s3://{get_settings.s3_bucket_name}/{s3_key}")